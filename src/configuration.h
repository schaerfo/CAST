  ...
  */
  ...
  struct energy
  {
    ...
    struct dftb_conf
    {
      ...
      std::string gradfile;
      /**name of file to external charges*/
      std::string ext_charges;
      ...
      bool opt;
      /**use charges from former calculation as initial charge guess*/
      bool init_charges;
      
      /**constructor
      for most options if a value is set to 0, the default values from dftbaby are used
      exceptions: gradstate, verbose*/
      dftb_conf(void): gradfile("grad.xyz"), gradstate(0), verbose(0), ext_charges("0"),
      longrange(false), cutoff(0), lr_dist(0), maxiter(0), conv_threshold("0"), init_charges(true),
