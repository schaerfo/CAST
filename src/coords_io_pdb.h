      ...
      struct helper : public helper_base {
        ...
        */
        ...
        public:
          ...
          /*static constexpr std::array<std::string, 26> RESIDUE_NAMES{ { "ALA", "ARG", "ASN", "ASP", "CYS", "GLN",
            "GLU", "GLY", "HIS", "ILE", "LEU", "LYS", "MET", "PHE", "PRO", "SER", "THR", "TRP", "TYR", "VAL",
            "CYX", "CYM", "CYP", "HID", "HIE", "HIP" } };*/
            /*!
        ...
        */
        ...
        public:
          ...
          \param vec std::vector of pdb::Atom
          \return std::vector of std::string
          */
          static std::vector<std::string> create_element_vec(std::vector<Atom_type> const& vec) {
          ...
          /*!
          \brief Uses a std::vector of pdb::Atom to form a std::vector of coords::Atom
          \param vec std::vector of pdb::Atom
          \return std::vector of coords::Atom
          */
          static coords::Atoms create_cooord_atoms(std::vector<Atom_type> const& vec) {
            coords::Atoms atoms;
            for (auto const& atom : vec) {
...
coords::input::formats::pdb::helper::get_terminals(Atoms const& atoms, std::vector<std::vector<std::size_t>> const& indices) {
  ...
  for (auto const& res_ind : indices) {
    ...
    for (auto const& ind : res_ind) {
      ...
      else if (a.get_pdb_atom_name() == "H1") tmp=terminals::N;
    }
...
void coords::input::formats::pdb::helper::set_energy_type(coords::Atoms& atoms, std::vector<std::vector<std::size_t>> const& indices) {
  ...
  for (auto i = 0u; i < indices.size(); ++i) {
    ...
    for (auto const& ind : res_ind) {
      ...
      int energy_type = find_energy_type(atom.get_pdb_atom_name(), atom.get_residue(), terminals[i]);
      if (energy_type == 0 && Config::get().general.energy_interface == config::interface_types::T::OPLSAA){
        ...
        throw std::runtime_error("Assigment of atom types failed. Please use another energy interface.\n");
      }
...
coords::Coordinates coords::input::formats::pdb::read(std::string const& file_name) {
